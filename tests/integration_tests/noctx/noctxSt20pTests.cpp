...
class St20pDefaultTimestamp : public FrameTestStrategy {
 public:
  ...
  void rxTestFrameModifier(void* frame, size_t frame_size) {
    ...
    uint64_t framebuffTime = st10_tai_to_media_clk(st20pParent->nsFrameTime, 90000);

    EXPECT_NEAR(f->timestamp,
                st10_tai_to_media_clk((idx_rx)*st20pParent->nsFrameTime, 90000),
                framebuffTime)
        << " idx_rx: " << idx_rx;
    if (lastTimestamp != 0) {
...
class St20pUserTimestamp : public St20pDefaultTimestamp {
 ...
 public:
  ...
  void rxTestFrameModifier(void* frame, size_t frame_size) {
    ...
    uint64_t expected_media_clk = st10_tai_to_media_clk(expectedTimestamp, 90000);

    EXPECT_EQ(f->timestamp, expected_media_clk)
        << " idx_rx: " << idx_rx << " tai difference: "
        << (int64_t)(st10_media_clk_to_ns(f->timestamp, 90000) - expectedTimestamp);

    if (lastTimestamp != 0) {
      uint64_t diff = f->timestamp - lastTimestamp;
      EXPECT_TRUE(diff == st10_tai_to_media_clk(st20pParent->nsFrameTime, 90000))
...
TEST_F(NoCtxTest, st20p_redundant_latency) {
  ...
  st20pHandlers[sessionRxSideId]->sessionsOpsTx.flags |= ST20P_TX_FLAG_USER_PACING;  // Changed from ST30P to ST20P
  ...
  st20pHandlers.emplace_back(
      new St20pHandler(ctx, sessionUserData, {}, {}, false, false));
  st20pHandlers[sessionTxPrimarySideId]->sessionsOpsTx.flags |= ST20P_TX_FLAG_USER_PACING;  // Changed from ST30P to ST20P
  ...
  */
  ...
  st20_rx_user_stats stats;  // Changed from st30_rx_user_stats to st20_rx_user_stats
  st20p_rx_get_session_stats(st20pHandlers[sessionRxSideId]->sessionsHandleRx, &stats);
  st20_tx_user_stats statsTxPrimary;  // Changed from st30_tx_user_stats to st20_tx_user_stats
  st20p_tx_get_session_stats(st20pHandlers[sessionTxPrimarySideId]->sessionsHandleTx,
                             &statsTxPrimary);
  st20_tx_user_stats statsTxRedundant;  // Changed from st30_tx_user_stats to st20_tx_user_stats
