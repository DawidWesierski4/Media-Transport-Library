...
static void ptp_delay_req_task(struct mt_ptp_impl* ptp) {
  ...
  if (ptp->t2_mode == MT_PTP_L4) {
    ...
    memcpy(ipv4_hdr, &ptp->dst_udp, sizeof(*ipv4_hdr));
  ...
  memcpy(&msg->hdr.source_port_identity, &ptp->our_port_id,
...
static int ptp_parse_announce(struct mt_ptp_impl* ptp, struct mt_ptp_announce_msg* msg,
  ...
  if (!ptp->master_initialized) {
    ...
    memcpy(&ptp->master_port_id, &msg->hdr.source_port_identity,
               sizeof(ptp->master_port_id));
    memcpy(&ptp->master_addr.addr_bytes[0], &ptp->master_port_id.clock_identity.id[0],
               3);
    memcpy(&ptp->master_addr.addr_bytes[3], &ptp->master_port_id.clock_identity.id[5],
               3);
    ...
    if (mode == MT_PTP_L4) {
      ...
      memcpy(dst_udp, ipv4_hdr, sizeof(*dst_udp));
      memcpy(&dst_udp->ip.src_addr, &ptp->sip_addr[0], MTL_IP_ADDR_LEN);
      memcpy(&dst_udp->ip.dst_addr, &ptp->mcast_group_addr[0], MTL_IP_ADDR_LEN);
...
static int ptp_init(struct mtl_main_impl* impl, struct mt_ptp_impl* ptp,
  ...
  memcpy(ip, mt_sip_addr(impl, port), MTL_IP_ADDR_LEN);
  ...
  if (mt_has_cni(impl, port) && !mt_drv_mcast_in_dp(impl, port)) {
    ...
    memcpy(flow.dip_addr, ptp->mcast_group_addr, MTL_IP_ADDR_LEN);
