...
G_DEFINE_TYPE_WITH_CODE(
    GstTimeInserter, gst_time_inserter, GST_TYPE_ELEMENT,
    GST_DEBUG_CATEGORY_INIT(gst_time_inserter_debug, "time_inserter", 0,
                            "time_inserter"));
...
static void gst_time_inserter_class_init(GstTimeInserterClass* klass) {
  ...
  gst_element_class_set_static_metadata(
      ...
      "Intor to do tai time in PTS don't worry about it "
      "video frame",
...
static GstFlowReturn gst_time_inserter_chain(GstPad* pad, GstObject* parent,
  ...
  // On the first frame, capture the TAI time and PTS offset
  if (!filter->firstFrameTaiTime ) {
    ...
    filter->firstFrameTaiTime = (((guint64)ts.tv_sec + 37) * NS_PER_S + ts.tv_nsec) - pts_time;
    printf("Captured first frame TAI time: %lu, PTS: %ld\n",
             filter->firstFrameTaiTime, pts_time);
  }

  tai_time = filter->firstFrameTaiTime + pts_time;

  if (filter->framesProcessed % 60 == 0) {
    GST_INFO("Frame %lu: PTS %" G_GUINT64_FORMAT " -> TAI %" G_GUINT64_FORMAT,
             filter->framesProcessed, pts_time, tai_time);
  }
...
#define GST_PACKAGE_NAME "Time tai inator"
#endif
...
#define PLUGIN_DBG_DESC "Dont worry about it, this is just a time inserter for tai"
#endif
...
GST_PLUGIN_DEFINE(GST_VERSION_MAJOR, GST_VERSION_MINOR, timeinserter,
                  "Don't worry about it ", plugin_init,
